...
public final class AppUtils {
											 ...
											 boolean isWriteToLogFile)
	{
		...
		if (isWriteToLogFile) {
			...
			String fileName = clazz.getSimpleName() + "_"
					+ DateUtil.format(new Date(), "yyyy-MM-dd") + ".log";

			String os = System.getProperty("os.name");
			String rootPath = "";
			String contextName = "/";
			if (os.toLowerCase().startsWith("win")) {
				File classPath = new File(clazz.getClassLoader()
						.getResource("").getPath());
				String[] contextPath = classPath.getParentFile().getParent()
						.split("\\\\");
				rootPath = "c:/whxxlogs";
			...
			String filePath = sb.append(rootPath).append(contextName)
					.append(clazz.getSimpleName()).append("/").append(fileName)
					.toString();
											 ...
											 boolean isWriteToLogFile)
	{
		...
		if (isWriteToLogFile) {
			...
			String contextName = "/";
			if (os.toLowerCase().startsWith("win")) {
				File classPath = new File(AppUtils.class.getClassLoader()
						.getResource("").getPath());
				String[] contextPath = classPath.getParentFile().getParent()
						.split("\\\\");
				rootPath = "c:/whxxlogs/";
	...
	public static org.slf4j.Logger getErrorLogger(String name,
												  boolean isWriteToLogFile)
	{
    ...
    public static List<ZTreeNode> buildZTreeNodeByRecursive(List<ZTreeNode> source, List<ZTreeNode> target, Predicate<ZTreeNode> predicate)
    {
        List<ZTreeNode> list = source.parallelStream().filter(predicate)
            .reduce(target, (u, t) -> {
                t.setChildren(buildZTreeNodeByRecursive(source, new ArrayList<>(), e -> Objects.equals(t.getId(), e.getPid())));
                u.add(t);
                return u;
            }, (u, t) -> u);
