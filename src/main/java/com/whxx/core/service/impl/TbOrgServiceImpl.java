...
import com.whxx.core.dao.TbManagerOrgMapper;
import com.whxx.core.dao.TbOrgMapper;
...
import com.whxx.core.service.ITbOrgService;
...
import static com.whxx.core.common.CoreConstant.ORG_FULLNAMES_SEPARATE;
...
public class TbOrgServiceImpl implements ITbOrgService
{
    ...
    public void updateItem(TbOrg org) throws Exception
    {
        ...
        if (Objects.equals(oldOrg.getParentCode(), org.getParentCode()))
        {
            ...
            oldOrg.setFullnames(StrUtil.replace(oldOrg.getFullnames(),
                StringUtils.lastIndexOf(oldOrg.getFullnames(), oldName),
                "oldName", org.getName(), false));
        ...
        else
        {
            ...
            if (behindOrg != null)
            {
                ...
                //查询出所有的关联的菜单
                List<TbOrg> children = tbOrgMapper.selectList(new QueryWrapper<TbOrg>().lambda().like(TbOrg::getFullcodes, "%" + oldCode + "%"));
                if (!children.isEmpty())
                {
    ...
    {
        TbOrg org = tbOrgMapper.selectById(orgId);
        TbOrg fontOrg = tbOrgMapper.selectOne(new QueryWrapper<TbOrg>()
            .lambda().lt(TbOrg::getSeq, org.getSeq())
            .eq(TbOrg::getParentCode, org.getParentCode())
    ...
    {
        TbOrg org = tbOrgMapper.selectById(orgId);
        TbOrg behindOrg = tbOrgMapper.selectOne(new QueryWrapper<TbOrg>()
            .lambda().gt(TbOrg::getSeq, org.getSeq())
            .eq(TbOrg::getParentCode, org.getParentCode())
            .last("order by seq asc limit 1")
        );
        if (behindOrg == null)
        {
