...
public class TbRoleServiceImpl implements ITbRoleService
{
    ...
    public void deleteRoleById(String roleId)
    {
        ...
        //删除该角色所有的权限
        List<String> roleMenuIds = tbRoleMenuMapper.selectList(new QueryWrapper<TbRoleMenu>().lambda().eq(TbRoleMenu::getRoleNum, role.getNum()))
        ...
        //删除与该角色关联的用户
        List<String> managerRoleIds = tbManagerRoleMapper.selectList(new QueryWrapper<TbManagerRole>().lambda().eq(TbManagerRole::getRoleId, role.getNum()))
    ...
    public void assignMenuForRole(String roleId, String menuIds)
    {
        ...
        //为角色配置新菜单
        Arrays.stream(menuIds.split(",")).filter(StringUtils::isNotEmpty)
            .forEach(e -> {
