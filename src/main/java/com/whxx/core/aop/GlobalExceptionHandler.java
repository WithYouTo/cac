...
import static com.whxx.core.utils.AppUtils.getLoginUserAccount;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<String> notFound(HttpServletRequest request, HttpServletResponse response, BussinessException e)
    {
        ...
        return specialConsumer(request, response, () -> new ResponseEntity<String>("", HttpStatus.INTERNAL_SERVER_ERROR));
    }
    ...
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    public ResponseEntity<String> notFound(HttpServletRequest request, HttpServletResponse response, RuntimeException e)
    {
    ...
    private void assertAjax(HttpServletRequest request, HttpServletResponse response) {
        if (request.getHeader(HttpHeaders.X_REQUESTED_WITH) != null && "XMLHttpRequest".equalsIgnoreCase(request.getHeader(HttpHeaders.X_REQUESTED_WITH)))
        {
    ...
    private static ResponseEntity<String> specialConsumer(HttpServletRequest request, HttpServletResponse response, Supplier<ResponseEntity> result)
    {
        Predicate<HttpServletRequest> p = (HttpServletRequest req) -> {
            return req.getHeader(HttpHeaders.ACCEPT).contains("application/json") || (req.getHeader(HttpHeaders.X_REQUESTED_WITH) != null && "XMLHttpRequest".equalsIgnoreCase(req.getHeader(HttpHeaders.X_REQUESTED_WITH)));
        };
        if (p.test(request))
        {
            return result.get();
